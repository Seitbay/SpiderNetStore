...
/**
 * Разрешает подписку только на {@code /topic/orders/{orderId}/chat} для участника заказа.
 */
@Component
@RequiredArgsConstructor
public class StompChatSubscribeInterceptor implements ChannelInterceptor {

    private static final Pattern ORDER_CHAT_TOPIC = Pattern.compile("^/topic/orders/(\\d+)/chat$");
    ...
    public Message<?> preSend(@NonNull Message<?> message, @NonNull MessageChannel channel) {
        ...
        Matcher m = ORDER_CHAT_TOPIC.matcher(dest);
        if (!m.matches()) {
            throw new AccessDeniedException("Недопустимый топик");
        }
